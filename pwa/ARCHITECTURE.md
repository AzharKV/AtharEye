...
**Last updated:** 2026-06-15 (SPEC §15 v1.25) · **Status:** **OptiSync redesign in progress** (branch
...
- **`lib/store.ts`** *(v1.25 — persisted + phased)* — `StoreProvider`/`useStore`; single mutation path `update(id, recipe, {rollup})` + `addProject`/`deleteProject`/`patch`. **Persists the full `AppData` + the demo `phase` to localStorage** (`optisync:state`, schema-versioned) on every change — refresh/relaunch keeps everything; **reset = clear the browser cache**. **Two runtime phases:** boots `client` (CK Group / Tabley); `deleteProject` that empties the list flips to `optisync` (swaps identity to Athar Robotics via `toOptisync`, projects empty) — a persisted one-way flag. Exposes `phase` + `demoNow` (phase-keyed). `VITE_DATASET=legacy` seeds the portfolio, never flips. On load `sanitize` drops dead `blob:`/`data:` capture URLs and **backfills `contractor`/`preparedBy` from the persisted account** so a report never falls back to the client-demo "Cairn / J. Mackay" (new projects get these from the account at creation in `NewProject`).
...
- **`App.tsx`** *(updated v1.20, v1.25)* — `AppRoot` holds `jobs: Record<string, ProcessingJob>` + `toastMsg`; `useEffect` timer (2 s tick) flips scan to `Ready` when `Date.now() - startedAt >= PROCESSING_MS`, clears job, fires toast. **v1.25:** a second effect **prunes jobs whose project was deleted** (e.g. the demo flip) and the timer only flips + toasts when the completing scan still exists — so no stray "report ready" toast fires for a removed project. `openNewProject` added (empty-state CTA). `GlobalToast` floats above tab bar (teal checkCircle, auto-hides 3 s).
- **`screens/editors.tsx`** — bottom-sheet CRUD editors + ScanLog/ScanDetail.
- **`screens/`** — `Projects` (list/search/filters/swipe-delete), `NewProject`, **`ProjectDetail`** (the scan-history scrubber is **labelled a coverage *simulation/preview*** as of v1.23 — it drives `stateAt`, which scales every zone proportionally to the scrubbed overall coverage, so it is honestly framed as "the site at X% overall", not real per-zone history; "View log" is the real per-scan record; scrubber nodes pulse amber when Processing; zone rows show amber "Processing" badge + amber coverage bar; `sevRank` handles 4-level severity; **v1.24 the Zone-coverage section + Add button always render on the latest view (empty state when 0 zones) so a fresh project can add its first area to scan**), **`Reports`** *(v1.21 major refactor)*: scope toggle "Whole project / By zone" + zone chips; **new layout order: donut → severity tally → zone bars → findings** above fold; meta in collapsible `CollapsibleSection`; `FindingCard` shows `thumbnail` scan image inline; **`ChangesSinceLastScan`** *(v1.23)* renders above the findings register in both scopes — previous→new coverage Δ + findings raised/resolved by the latest scan (reads the frozen `Scan` snapshot; considers Ready + Processing scans so a just-recorded live scan shows its delta immediately; hidden for a first-time/baseline report); `NsrRollup` (project scope: X/Y complete + per-zone counts); `NsrTable` (zone scope: full code/description/unit/qty/status table); zone scope adds `ZoneEvidenceFrame` (poster from `zoneMedia`); pending = slim `ProcessingBanner` when zone has prior data (not blank screen); `NsrItem`/`NsrStatus` from `types.ts`; `Account`, `Issues`, `Plans`, `Team`, `Settings`, **`scan/ScanFlow`** *(v1.22 fix; v1.23 OptiSync preset)*: `step === 'result'` shows "Capture complete · {zone} · ready to upload" + "Upload & finish" only (BIM alignment is server-side; deltas revealed at Ready). **v1.23/v1.25 (`optisync = store phase === 'optisync'`):** the camera steps attach the **live rear camera** (`getUserMedia` → `video.srcObject`) behind the point-cloud overlay. **v1.25:** the seeded `<video src/poster>` is dropped in this phase (no mismatched-room clip); on permission **deny/unavailable** a retryable **"Camera access needed"** card (`camState`) shows instead of a fallback video. A `'mismatch'` step resolves the upload into a branded red **`BimMismatchPanel`** ("Scan doesn't match BIM model · no alignment found") instead of committing a coverage scan. All gated on the phase, so the `client` phase keeps the seeded walkthrough clip. `DeltaTile` helper removed.
...
- **Persisted to localStorage** (`lib/store.ts`, v1.25) seeded from `data.ts` — created projects, scans, edits + identity survive reload; reset by clearing the browser cache. Single build, two runtime phases (`client` → delete project → `optisync`) via a persisted flag (no `VITE_DEMO`). No backend. (Firebase/etc. is the future multi-device path.)
...
| Live camera vs seeded clip in scan | `OPTISYNC` flag in `scan/ScanFlow.tsx` — on (`VITE_DEMO=optisync`) = live `getUserMedia` rear camera behind the point cloud; off = the seeded `zoneMedia(zone.id)` walkthrough clip |
