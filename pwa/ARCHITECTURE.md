...
- **`lib/pdf.ts`** *(new v1.20, updated v1.21)* — `generatePdf(report, scope, activeZoneId?)` → `Promise<Blob>`. Lazy jsPDF import (not in main bundle). **v1.21:** ~3 pages (cover+exec combined on p1; zone progress page skipped for zone-scope; NSR works page; methodology+sign-off). Scoped coverage: zone export uses zone %, project export uses project %. Scan evidence thumbnails embedded per finding card (fetched as data URL, `addImage`). NSR works table (zone: single zone; project: rollup + per-zone). No dev language. All colours locked tokens only.
