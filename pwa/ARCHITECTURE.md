...
- **`lib/store.ts`** *(v1.25 — persisted + phased)* — `StoreProvider`/`useStore`; single mutation path `update(id, recipe, {rollup})` + `addProject`/`deleteProject`/`patch`. **Persists the full `AppData` + the demo `phase` to localStorage** (`optisync:state`, schema-versioned) on every change — refresh/relaunch keeps everything; **reset = clear the cache or `resetDemo()`**. **Two runtime phases:** boots `client` (CK Group / Tabley); `deleteProject` that empties the list flips to `optisync` (swaps identity to Athar Robotics via `toOptisync`, projects empty) — a persisted one-way flag. Exposes `phase` + `demoNow` (phase-keyed). `VITE_DATASET=legacy` seeds the portfolio, never flips. Dead `blob:`/`data:` capture URLs dropped on load.
...
- **Persisted to localStorage** (`lib/store.ts`, v1.25) seeded from `data.ts` — created projects, scans, edits + identity survive reload; reset by clearing the cache / `resetDemo`. Single build, two runtime phases (`client` → delete project → `optisync`) via a persisted flag (no `VITE_DEMO`). No backend. (Firebase/etc. is the future multi-device path.)
